...
if $seal_secrets; then
  ...
  # Sealed-*.yaml files go INTO platform-overlay/ (the consumer overlay's
  # Kustomize dir) so the root flux Kustomization doesn't double-include
  # them (it auto-scans top-level *.yaml; platform-overlay/kustomization.yaml
  # ALSO lists them — duplicate registration would break the build).
  ...
  # GHCR image-pull secret — three namespaces:
  #  - dc-system:      used by dc-api/cloud-ui/postgres Deployments' imagePullSecrets
  #  - flux-system:    used by ImageRepository CRs to scan private GHCR
  #  - keyvault-system: used by keyvault-operator's Deployment imagePullSecret
  # sealed-secrets is namespace-scoped, so one SealedSecret per namespace.
  ...
  # Match the template's sealed-secrets banner — accept either the old
  # wording ("the wizard drops") or the new wording ("generated by
  # scripts/init-flux.sh") so older overlays don't silently no-op.
  awk '
